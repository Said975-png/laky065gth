  ...
  try {
    ...
    if (!groqApiKey) {
      ...
      if (
        ...
        fallbackMessage =
          "👋 Привет! Я Пятница, ваш ИИ-консультант. С��йчас работаю в демо-режиме. Расскажу о наших тарифах веб-разработки!";
      } else if (
    ...
    console.log("🚀 Отправляем запр��с к Groq API...");
...
2. 🥈 PRO - 3.500.000 сум (скидка с 4.000.000):
   ...
   🤖 ИИ-чат бот для автоматических ответов
   ...
   🔐 Система авторизации пол��зователей
...
- Используй эмодзи умеренно для удобства восприятия
...
ВАЖНО: Ты можешь общаться на ЛЮБЫЕ темы - от программирования до кулинарии, от науки до развлечений. Главное - быть полезным помощником!`,
    ...
    if (!groqResponse.ok) {
      ...
      if (
        ...
        fallbackMessage =
          "👋 Привет! Я Пятн��ца, ваш ИИ-консультант по веб-разработке. В данный момент основной API недоступен, но я могу рассказать о наших тарифах!\n\n🥉 BASIC - 2.500.000 сум\n🥈 PRO - 3.500.000 сум\n🥇 MAX - 5.500.000 сум\n\nЧто вас интересует?";
      } else if (
        lastMessage.includes("тариф") ||
        lastMessage.includes("цен") ||
        lastMessage.includes("стоимость")
