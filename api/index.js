export default async function handler(req, res) {
  ...
  try {
    ...
    if (url === '/api/groq-chat' && method === 'POST') {
      ...
      if (lastMessage.includes("привет") || lastMessage.includes("здравствуй") || lastMessage.includes("hello")) {
        intelligentResponse = "👋 Привет! Я Пятница, ваш персональный AI-помощник от Stark Industries. Готова помочь вам с любыми вопросами!";
      } else if (lastMessage.includes("как дела") || lastMessage.includes("что делаешь")) {
        intelligentResponse = "🤖 Отлично! Анализирую данные и готова к работе. Чем могу быть полезна?";
      } else if (lastMessage.includes("что умеешь") || lastMessage.includes("функции") || lastMessage.includes("возможности")) {
        intelligentResponse = "💡 Мои возможности:\n\n🔹 Консультации по веб-разработке\n🔹 Помощь с техническими вопросами\n🔹 Анализ и рекомендации\n🔹 Поддержка по продуктам\n🔹 Общение и ответы на вопросы\n\nЗадавайте любые вопросы!";
      } else if (lastMessage.includes("пока") || lastMessage.includes("до свидания") || lastMessage.includes("bye")) {
        intelligentResponse = "👋 До свидания! Было приятно пообщаться. Обращайтесь снова, когда понадобится помощь!";
      } else if (lastMessage.includes("сайт") || lastMessage.includes("веб") || lastMessage.includes("разработка")) {
        intelligentResponse = "🌐 Отличный вопрос о веб-разработке! Мы в Stark Industries создаем современные сайты с:\n\n✨ Адаптивным дизайном\n⚡ Высокой производительностью\n🎨 Уникальными решениями\n🔒 Надежной безопасностью\n\nРасскажите подробнее о вашем проекте!";
      } else if (lastMessage.includes("цена") || lastMessage.includes("стоимость") || lastMessage.includes("сколько")) {
        intelligentResponse = "💰 Стоимость зависит от сложности проекта:\n\n📱 Лендинг: от 50,000₽\n🌐 Корпоративный сайт: от 150,000₽\n🛒 Интернет-магазин: от 300,000₽\n⚡ Индивидуальные решения: по запросу\n\nДля точной оценки нужно обсудить ваши требования!";
      } else if (lastMessage.includes("время") || lastMessage.includes("сроки") || lastMessage.includes("когда")) {
        intelligentResponse = "⏰ Типичные сроки разработки:\n\n📱 Лендинг: 1-2 недели\n🌐 Корпоративный сайт: 3-6 недель\n🛒 Интернет-магазин: 6-12 недель\n\nТочные сроки зависят от сложности и требований проекта.";
      } else if (lastMessage.includes("технологи") || lastMessage.includes("стек") || lastMessage.includes("инструменты")) {
        intelligentResponse = "🔧 Мы используем современный технологический стек:\n\n⚛️ React, TypeScript, Next.js\n🎨 TailwindCSS, Framer Motion\n⚡ Node.js, Express\n📊 PostgreSQL, MongoDB\n☁️ Vercel, AWS, Docker\n\nВыбираем оптимальные технологии под каждый проект!";
      } else if (lastMessage.includes("контакт") || lastMessage.includes("связ") || lastMessage.includes("телефон")) {
        intelligentResponse = "📞 Свяжитесь с нами:\n\n📧 Email: contact@stark-ai.com\n📱 Telegram: @stark_support\n🌐 Форма на сайте: /order\n\nМы ответим в течение часа!";
      } else if (lastMessage.includes("jarvis") || lastMessage.includes("джарвис")) {
        intelligentResponse = "🤖 Да, я Jarvis - ваш AI-помощник! Названа в честь легендарного ИИ Тони Старка. Готова помочь с любыми техническими вопросами и задачами!";
      } else {
