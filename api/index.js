export default async function handler(req, res) {
  ...
  try {
    ...
    if (url === "/api/contracts" && method === "GET") {
      ...
      // Возвращаем пустой ��ассив для демо (в реальности здесь была бы база данных)
      return res.json({
        success: true,
        contracts: [],
      });
    ...
    if (url === "/api/groq-chat" && method === "POST") {
      ...
      if (
        ...
        if (
          ...
          intelligentResponse =
            "🤖 Отличный вопрос! Я стараюсь анализировать контекст и предоставлят�� максимально полезную информацию. Могу помочь с техническими вопросами, консультациями по разработке, или просто поболтать. Что именно вас интересует?";
        } else if (
          ...
          intelligentResponse =
            "🧠 Хороший вопрос! Анализируя ваш запрос, постараюсь д��ть развернутый ответ. Если это касается наших т��хнологий или услуг - с радостью расскажу подробности. Уточните, пожалуйста, что именно вас интересует?";
        } else if (
          lastMessage.includes("когда") ||
          ...
          const responses = [
            "🤔 Интересный вопрос! Хотя я специализиру��сь на технических консультациях, постараюсь помочь. Расскажите подробнее о том, что вас интересует?",
    ...
    if (url === "/api/bookings" && method === "POST") {
      console.log("📅 [BOOKING] Создание брони - запрос получен");
      ...
      console.log("✅ [BOOKING] Бронь создана:", bookingId);
    ...
    if (url === "/api/bookings" && method === "GET") {
      console.log("📋 [BOOKING] Получение броней пользователя");
      const userId = req.headers["user-id"];
    ...
    if (url === "/api/bookings/all" && method === "GET") {
      console.log("🔐 [BOOKING] Получение всех броней (админ)");
    ...
    if (url.startsWith("/api/bookings/") && method === "PUT") {
      ...
      console.log("📝 [BOOKING] Обновление брони:", bookingId);
    ...
    if (url.startsWith("/api/bookings/") && method === "DELETE") {
      const bookingId = url.split("/api/bookings/")[1];
      console.log("🗑️ [BOOKING] Удаление брони:", bookingId);
