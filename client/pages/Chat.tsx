...
export default function Chat() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      let errorText = "Извините, произошла ошибка сети. Попробуйте еще раз.";

      if (error instanceof Error) {
        if (error.message.includes("Failed to fetch")) {
          errorText =
            "🌐 Проблема с подключением к серверу. Проверьте интернет и попробуйте снова.";
        } else if (error.message.includes("HTTP error")) {
          errorText =
            "⚠️ Сервер временно недоступен. Попробуйте через несколько минут.";
        } else if (error.message.includes("Неверный формат")) {
          errorText =
            "🔧 Проблема с форматом данных. Обновите страницу и попробуйте снова.";
        }
      }
